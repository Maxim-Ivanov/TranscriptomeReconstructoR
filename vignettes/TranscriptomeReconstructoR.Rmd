...
For more details on downloading, pre-processing and alignment of the raw sequencing data, **see Appendix 3**. The BAM files were subsetted to a 300Kb interval on chromosome 1 (1:10,000,000-10,300,000). To retrieve the filenames, run the following code:
...
If by any reason you are not satisfied with the results of peak calling, then changing the default values of `call_TCs` arguments is recommended. In particular, if the input data are noisy (i.e. have high non-specific background), then the results may be improved by using higher `min_tpm`, `q_trim` and `min_score` values. Alternatively, any other peak calling software can be used instead of `call_TCs` (e.g. `r Biocpkg("CAGEr")` or `r Biocpkg("CAGEfightR")`), given that the output is converted to *GRanges* with a valid `score` column.
...
# Appendix 3. Download and align the raw data
...
for file in ont_rep{1..4}.fq.gz; do echo $file && minimap2 -t 8 -ax splice -k 14 -L --cs --secondary=no -G 10000 TAIR10.fa $file > ${file/fq.gz/sam}; done
...
for file in planet*mapq.bam; do echo $file && samtools index $file && umi_tools dedup --stdin=${file} --stdout=${file/_sorted_mapq.bam/_final.bam} --paired; done
```
...
for file in *final.bam; do echo $file && samtools view -hb $file "1:10000000-10300000" > ${file/_final/_filt}; done
```
